...
simpars$alpha <- 2 #Type one (positive) number
plotOUtreesim(simpars, tree)
abline(h = simpars$theta, col=rainbow(3))
...
simpars$theta <- c(0,1,-1) #Type simpars$ntheta numbers
...
#large alpha loses phylogenetic structure -- white noise ~ radiation
#flat likelihood surfaces: brownian motion or no phylogeny
...
```{r}
#get distribution of phenotypes
#can we recover shifts from tip data?
...
(Allowing a range from ~ white noise to ~brownian motion)
```{r}
...
mcmcOU <- bayou.makeMCMC(tree, dat, SE=MEvar, prior=priorOU, 
                         new.dir="modelOU/", outname="modelOU_r001", plot.freq=NULL) # Set up the MCMC
...
Our traces will probably look bad, 10,000 generations isn't long enough to obtain convergence. Also, note the small effective sample sizes in our summary (the NA's for the *all theta* row are expected, this is because these aren't a single parameter, but a variable number of optima that are coming in and out of existence throughout the chain). For a good run: need ~200k or ideally ~1M
