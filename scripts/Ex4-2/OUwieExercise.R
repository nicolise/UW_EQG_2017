...
#lnL = log likelihood, AIC and AICc for fit
#calculates alpha (wiggle around optimum) and sigma squared()
...
?OUwie.fixed #give me likelihood at specific points -- get the curve
...
alpha.values<-seq(from= 0.0001, to= 2 , length.out=50)
...
plot(x= alpha.values , y= likelihood.values, xlab="???", ylab="???", type="l", bty="n")
...
#if 1 parameter at at a time: you might think you have a point optimum
#if a ridge (line of optima): you might find the ridge through this plotting
#look at 2 parameters at a time: more conservative estimate of uncertainty!
