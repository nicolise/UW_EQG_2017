...
thamnophis = read.table('data/Ex2-2/Radix5_2012.txt',header=TRUE)
...
#We want to standardize BODY and TAIL so that their means are zero and their sds are 1 (z-scale)
...
# y ~ beta1 + beta2 + gamma11 + gamma22 + gamma12
#if gammas significant, shows stabilizing selection on trait
#if gamma12 significant, 
...
#beta values: selected from the simple linear fit (the linear approximation is more robust to skewness in the distribution vs. in the quadratic fit)
#gamma values: selected from the quadratic fit
beta1= 0.031408  
beta2= 0.001504
gamma11 = -0.010578 
gamma22 = -0.005914 
...
jet.colors <- colorRampPalette( c("yellow", "turquoise") )
...
for (i in 1:100){
  ...
  Sys.sleep(0.1) #only draws a new plot every 0.1 sec so user can see it
}
...
par(pty ="s")
filled.contour(x,y,z) #this is pretty but does not align correctly with the stationary point and eigenvectors
contour(x, y, z)
...
omega=-(solve(gamma))
omega
#omega is like a variance term in a gaussian distribution (though here we have an ~upside down gaussian surface)
#Its eigenvectors should be the same as those of the  negative gamma-matrix
#Let's check
eigen(omega)
