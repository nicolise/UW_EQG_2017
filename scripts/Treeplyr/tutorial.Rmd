...
td[[1]]
td[['X1']] #trait name
td[1:10,1:2] #which columns and rows
```
...
``` {r }
select(td, X1, D1) #tree data object but only for the columns we ask for
select(td, 1:3)
select(td, 1, 4, 6)
```
...
#and drops all tips that don't match the conditions
```
...
``` {r }
#creates a new trait, and you can name it in the command
...
``` {r }
#do everything at once, get really fancy! output of first is input for second... etc.
